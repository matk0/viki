...
func TestGeneratedTypesContainContractEnums(t *testing.T) {
	...
	checks := []struct {
		...
		{"Go", string(goTypes), []string{"type PageKind string", `PageKind = "primitive"`, `PageKind = "subscenario"`, "type BDDKeyword string", `BDDKeyword = "given"`, `BDDKeyword = "but"`, "type AssistantConversation struct", "type AssistantStatus struct"}},
		{"TypeScript", string(typescriptTypes), []string{`PageKind: "primitive" | "scenario" | "subscenario"`, `BDDKeyword: "given" | "when" | "then" | "and" | "but"`, `AssistantMode: "qa" | "edit"`, `AssistantConversation:`, `AssistantStatus:`}},
	}
